...
public class IngredientController {
  ...
  public ResponseEntity<Object> createIngredient(@Valid @RequestBody IngredientRequestDTO ingredientRequestDTO) {
    if (ingredientService.existsIngredientByName(ingredientRequestDTO.name())) {
      ...
      response.put("message", "Ingredient with this name already exists!");
      return ResponseEntity.badRequest().body(response);
    }

    // 2. Fetch the FULL category (ID + NAME) from the database
    IngredientCategory category = categoryService.getCategoryById(ingredientRequestDTO.ingredientCategoryId())
            .orElseThrow(() -> new IllegalArgumentException("Category does not exist!"));

    // 3. Map DTO → Ingredient (now includes category name)
    Ingredient newIngredient = IngredientMapper.toIngredient(ingredientRequestDTO, category);
  ...
  public ResponseEntity<Void> deleteIngredient(@PathVariable Long id) {
    Optional<Ingredient> ingredientOptional = this.ingredientService.findIngredientById(id);
